...
kubectl delete externalsecret postgres-credentials -n "3-tier-webapp-${ENVIRONMENT}" --ignore-not-found=true || true

# Delete SecretStore
kubectl delete secretstore azure-keyvault-store -n "3-tier-webapp-${ENVIRONMENT}" --ignore-not-found=true || true
