...
kubectl wait --for=condition=Healthy application/3-tier-webapp-${ENVIRONMENT} -n ${ARGOCD_NAMESPACE} --timeout=180s || true

# Show application status
kubectl get application 3-tier-webapp-${ENVIRONMENT} -n ${ARGOCD_NAMESPACE} || true
