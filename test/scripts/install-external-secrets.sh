...
for i in {1..30}; do
  if kubectl get secret postgres-credentials-from-kv -n "3-tier-webapp-${ENVIRONMENT}" >/dev/null 2>&1; thenbectl get secret postgres-credentials-from-kv -n "3-tier-webapp-${ENVIRONMENT}" >/dev/null 2>&1; then
